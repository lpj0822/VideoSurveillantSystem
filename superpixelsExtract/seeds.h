...
public:
    ...
    void update_image_ycbcr(unsigned int* image);
	
	// go through iterations
	void iterate();

    unsigned int* get_labels() { return labels[seeds_top_level]; }
	// output labels
    unsigned int** labels;	 //[level][y * width + x]

	// evaluation
	int count_superpixels();
    void SaveLabels_Text(std::string filename);
...
private:
	...
	float* image_a;
	float* image_b;
    unsigned int* image_bins; //bin index (histogram) of each image pixel [y*width + x]
    ...
    unsigned int* nr_labels; //[level]
    unsigned int** parent; //[level][label] = corresponding label of block with level+1
    unsigned int** nr_partitions; //[level][label] how many partitions label has on level-1
    ...
    int threebythree(int x, int y, unsigned int label);
    int threebyfour(int x, int y, unsigned int label);
    int fourbythree(int x, int y, unsigned int label);
