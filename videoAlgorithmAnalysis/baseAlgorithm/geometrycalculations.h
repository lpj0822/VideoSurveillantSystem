...
public:
    ...
    bool pointInPolygon(cv::Point2f point,const std::vector<cv::Point>& polygon);
    bool pointInPolygon(cv::Point point,const std::vector<cv::Point2f>& polygon);
    //得到多边形的中心
    cv::Point getPolygonCenter(const std::vector<cv::Point>& polygon);
    void pointsOrdered(std::vector<cv::Point>& polygon);//根据点的角度对多边形的点排序
    //计算多边形的面积
    ...
    //将矩形转换为点存储
    std::vector<cv::Point> covRectPolygon(cv::Rect rect);
    //计算相交的多边形
