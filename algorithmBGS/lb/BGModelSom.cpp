      ...
      for (int j = 0; j < m_height; j++)
      {
        ...
        for (int i = 0; i < m_width; i++)
        {
          ...
          if (d2min <= epsilon) // matching model found
          {
            ...
            prgbBG[j][i].Red = (unsigned char)m_ppSOM[jjHit][iiHit].Red;
            prgbBG[j][i].Green = (unsigned char)m_ppSOM[jjHit][iiHit].Green;
            prgbBG[j][i].Blue = (unsigned char)m_ppSOM[jjHit][iiHit].Blue;
