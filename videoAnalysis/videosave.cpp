...
void VideoSave::saveVideoData(QString videoPath,QString savePath,QString saveFileName,int startPos,int intervalSec)
{
    int errCode;
    ...
    if(saveThread[numThread%MAXVIDEOTHREAD]==NULL)
    {
        ...
        if(errCode==0)
        {
            errCode=saveThread[numThread%MAXVIDEOTHREAD]->initSaveVideoData(savePath,saveFileName,startPos,intervalSec);
            if(errCode==0)
            {
...
void VideoSave::saveVideoData1(QString videoPath, QString savePath, QString saveFileName, int stopPos, int intervalSec)
{
    int errCode;
    if(videoPath.isEmpty()||savePath.isEmpty()||saveFileName.isEmpty())
    {
...
void VideoSave::saveVideoData2(QString videoPath, QString savePath, QString saveFileName, int stopSec, int intervalSec)
{
    int errCode;
    if(videoPath.isEmpty()||savePath.isEmpty()||saveFileName.isEmpty())
    {
    ...
    if(saveThread[numThread%MAXVIDEOTHREAD]==NULL)
    {
        ...
        if(errCode==0)
        {
            errCode=saveThread[numThread%MAXVIDEOTHREAD]->initSaveVideoData1(savePath,saveFileName,stopSec,intervalSec,true);
            if(errCode==0)
            {
                saveThread[numThread%MAXVIDEOTHREAD]->startThread();
                saveThread[numThread%MAXVIDEOTHREAD]->start();
