...
static inline float diff(cv::Mat &img3f, int x1, int y1, int x2, int y2)
{
...
*/
int segmentImage(cv::Mat &srcFC3, cv::Mat &pImgInd, double sigma, double c, int min_size)
{
    CV_Assert(srcFC3.type() == CV_32FC3);
    int width(srcFC3.cols), height(srcFC3.rows);
    cv::Mat smImg3f;
    cv::GaussianBlur(srcFC3, smImg3f, cv::Size(), sigma, 0, cv::BORDER_REPLICATE);
