...
CmAPCluster::CmAPCluster(): dlh(NULL)
{
    if (!(dlh=LoadLibrary(L"E:/Qt5/GCSaliency/cluster/apclusterwin.dll"))) // Download the corresponding dll from (Notice: only 32bit dll is public available at the moment): http://www.psi.toronto.edu/index.php?q=affinity+propagation
...
int CmAPCluster::ReMapIdx(std::vector<int> &mapIdx)
{
	int N = (int)mapIdx.size(), newCount = 0;
    std::map<int, int> idxCount, oldNewIdx;
    std::vector<int> newIdx(N);
	for (int i = 0; i < N; i++){
