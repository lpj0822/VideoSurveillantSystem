...
public:
    ...
    inline float P(const cv::Vec<float, D> &c) const {return P(c.val);}

    // Returns the probability density of color c in just Gaussian k
    inline double P(int i, const float c[D]) const;
    inline double P(int i, const cv::Vec<float, D> &c) const {return P(i, c.val);}

    //return the mean color of component k
    cv::Vec<float, D> getMean(int k) const;
    ...
    void BuildGMMs(cv::Mat& sampleDf, cv::Mat& component1i, cv::Mat& w1f = cv::Mat());
    int RefineGMMs(cv::Mat& sampleDf, cv::Mat& components1i, cv::Mat& w1f = cv::Mat(), bool needReAssign = true); // Iteratively refine GMM

    bool Save(std::string &name) const;
    bool Load(std::string &name);
    double GetSumWeight() const {return _sumW;}

    void GetProbs(cv::Mat sampleDf, std::vector<cv::Mat> &pci1f) const; // Get Probabilities of each Channel i
    void GetProbsWN(cv::Mat sampleDf, std::vector<cv::Mat> &pci1f) const; // Get Probabilities of each Channel i, without normalize

    void iluProbs(cv::Mat sampleDf, std::string &nameNE) const; // Get Probabilities of each Channel i, and illustrate it, without normalize
    void iluProbsWN(cv::Mat sampleDf, std::string &nameNE) const; // Get Probabilities of each Channel i, and illustrate it, without normalize
...
protected:
    ...
    void AssignEachPixel(cv::Mat& sampleDf, cv::Mat &component1i);
};
...
public:
    ...
    static double ViewFrgBkgProb(const CmGMM &fGMM, const CmGMM &bGMM);

    // Show GMM images
    void Show(cv::Mat& components1i);

    void reWeights(std::vector<double> &mulWs);
};
...
// Add a color sample
template <int D> template<typename T>
void CmGaussianFitter<D>::Add(const T* _c)
{
    double c[D];
    for (int i = 0;  i < D; i++)
        c[i] = _c[i];
...
template <int D> template<typename T>
void CmGaussianFitter<D>::Add(const T* _c, T _weight)
{
    double c[D];
    for (int i = 0;  i < D; i++)
        c[i] = _c[i];
...
// Build the Gaussian out of all the added color samples
template <int D>
void CmGaussianFitter<D>::BuildGuassian(CmGaussian<D>& g, double totalCount, bool computeEigens) const
{
    ...
    else
    {
        ...
        if (computeEigens)
        {
            cv::Mat eVals(D, 1, CV_64FC1, g.eValues);
            cv::Mat eVecs(D, D, CV_64FC1, g.eVectors);
            cv::Matx<double, D, D> tmp;
            cv::SVD::compute(covar, eVals, eVecs, tmp);
        }
    }
}
...
/************************************************************************/
template <int D>
CmGMM_<D>::CmGMM_(int K, double thrV) : _K(K), _ThrV(thrV), _MaxK(K)
{
    _Guassians = new CmGaussian<D>[_K];
}

template <int D>
CmGMM_<D>::~CmGMM_(void)
{
    if (_Guassians)
        delete []_Guassians;
}
...
template <int D>
float CmGMM_<D>::P(const float c[D]) const
{
    double r = 0;
    if (_Guassians)
        for (int i = 0; i < _K; i++)
...
template <int D>
double CmGMM_<D>::P(int i, const float c[D]) const
{
    double result = 0;
...
template <int D>
void CmGMM_<D>::BuildGMMs(cv::Mat& sampleDf, cv::Mat& component1i, cv::Mat& w1f)
{
    bool weighted = w1f.data != NULL;
    int rows = sampleDf.rows, cols = sampleDf.cols;
    component1i = cv::Mat::zeros(sampleDf.size(), CV_32SC1);
    {
        ...
        _sumW = weighted ? cv::sum(w1f).val[0] : rows * cols; // Finding sum weight
    }
...
template <int D>
int CmGMM_<D>::RefineGMMs(cv::Mat& sampleDf, cv::Mat& components1i, cv::Mat& w1f, bool needReAssign)
{
    bool weighted = w1f.data != NULL;
    int rows = sampleDf.rows, cols = sampleDf.cols;
    {
...
template <int D>
void CmGMM_<D>::AssignEachPixel(cv::Mat& sampleDf, cv::Mat &component1i)
{
    int rows = sampleDf.rows, cols = sampleDf.cols;
...
template <int D>
cv::Vec<float, D> CmGMM_<D>::getMean(int k) const
{
    CV_Assert(k >= 0 && k <= _K);
    cv::Vec3f meanColor;
...
template <int D>
double CmGMM_<D>::getWeight(int k) const
{
    if (k < 0 || k >= _K)
        return 0.0;
...
template <int D>
bool CmGMM_<D>::Save(std::string &name) const
{
...
template <int D>
bool CmGMM_<D>::Load(std::string &name)
{
...
template <int D>
void CmGMM_<D>::GetProbs(cv::Mat sampleDf, std::vector<cv::Mat> &pci) const
{
    pci.resize(_K);
    cv::Mat pI = cv::Mat::zeros(sampleDf.size(), CV_32FC1);
    for (int c = 0; c < _K; c++)
    {
        // for each component c
        pci[c].create(sampleDf.size(), CV_32FC1);
    ...
    for (int c = 0; c < _K; c++)
        cv::divide(pci[c], pI, pci[c]);
}

template <int D>
void CmGMM_<D>::GetProbsWN(cv::Mat sampleDf, std::vector<cv::Mat> &pci) const
{
    pci.resize(_K);
    for (int c = 0; c < _K; c++)
    {
...
template <int D>
void CmGMM_<D>::iluProbs(cv::Mat sampleDf, std::string &nameNE) const
{
    std::vector<cv::Mat> pci;
    GetProbs(sampleDf, pci);
    cv::Mat tmpShow;
    for (int i = 0; i < _K; i++)
    {
        cv::normalize(pci[i], tmpShow, 0, 255, cv::NORM_MINMAX, CV_8UC1);
    }
}

template <int D>
void CmGMM_<D>::iluProbsWN(cv::Mat sampleDf, std::string &nameNE) const
{
    std::vector<cv::Mat> pci;
    ...
    for (int i = 0; i < _K; i++)
    {
        ...
        char* str1;
        char* str2;
        sprintf(str1,"Min = %g, ",minVal);
        sprintf(str2,"Max = %g", maxVal);
        cv::putText(tmpShow, str1, cv::Point(5, 20), CV_FONT_HERSHEY_PLAIN, 1, CV_RGB(255, 0, 0));
        cv::putText(tmpShow, str2, cv::Point(5, 40), CV_FONT_HERSHEY_PLAIN, 1, CV_RGB(255, 0, 0));
    }
}
