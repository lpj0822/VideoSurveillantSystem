...
void solve_tridiagonal_in_place_destructive(std::vector<double> &x, const size_t N, std::vector<double> &a, std::vector<double> &b, std::vector<double> &c);
...
void FGS_simple(cv::Mat_<float> &image, cv::Mat_<cv::Vec3f> &joint_image, double sigma, double lambda, int solver_iteration, double solver_attenuation)
{
    ...
    std::vector<double> a_vec(width,0),b_vec(width,0),c_vec(width,0),x_vec(width,0),c_ori_vec(width,0);
    std::vector<double> a2_vec(width,0),b2_vec(width,0),c2_vec(width,0),x2_vec(width,0),c2_ori_vec(width,0);
