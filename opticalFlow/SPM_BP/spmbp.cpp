...
void spm_bp::loadPairs(cv::Mat& in1, cv::Mat& in2)
{
    in1.copyTo(im1d);
    in2.copyTo(im2d);
...
inline bool checkEqual_PMF_PMBP(cv::Vec2f v1, cv::Vec2f v2)
{
    return (fabs(v1[0] - v2[0]) <= EPS) && (fabs(v1[1] - v2[1]) <= EPS);
}

inline int isNewLabel_PMF_PMBP(const std::vector<cv::Vec2f>& vec_label, const cv::Vec2f& test_label)
{
    int vec_size = (int)vec_label.size();
...
inline float ComputeMes_PMBP_per_label(const float* dis_belief,
                                       const cv::Mat_<cv::Vec2f>& label_k,
                                       int p, 
                                       const cv::Vec2f& disp_ref,
                                       float wt, float tau_s)
{
    ...
    for (int k = 0; k < NUM_TOP_K; ++k) {
        ...
        float cost_tp = dis_belief[k] + wt * std::min(float(abs(disp_ref[0] - label_k[p][k][0]) + abs(disp_ref[1] - label_k[p][k][1])), tau_s);
#endif

#if SMOOTH_COST_TRUNCATED_L2
        float cost_tp = dis_belief[k] + wt * std::min((float)(pow(disp_ref[0] - label_k[p][k][0], 2) +
...
inline void Compute_top_k(
        std::vector<float>& vec_belief,
        std::vector<cv::Vec2f>& vec_label,
        std::vector<float>& vec_mes_l, std::vector<float>& vec_mes_r, std::vector<float>& vec_mes_u, std::vector<float>& vec_mes_d,
        std::vector<float>& vec_d_cost,
        cv::Mat_<cv::Vec<float, NUM_TOP_K> >& mes_pixel,
        cv::Mat_<cv::Vec2f>& label_pixel,
        cv::Mat_<float>& d_cost,
        int p, int num_top_k)
{
...
void Message_normalization_PMF_PMBP(cv::Mat_<cv::Vec<float, NUM_TOP_K> >& mes_pixel, int p, int num_top_k)
{
    ...
    for (int i = 0; i < 4; i++) {
        ...
        // If that is the case we can just sum over std::vector to get val
...
cv::Vec2f WTA_PerPixel_Min_PMF_PMBP(float* pCost2, cv::Mat_<cv::Vec2f>& plabel, int p, int num_top_k)
{
...
void spm_bp::runspm_bp(cv::Mat_<cv::Vec2f>& flowResult)
{
    ...
    cv::Mat_<cv::Vec2f> label_k(height1 * width1, NUM_TOP_K);
    cv::Mat_<float> dcost_k(height1 * width1, NUM_TOP_K);

#ifdef _WIN32
    printf("Running on Windows\n");
    // Dummy code that speeds up program on windows considerably
    // Save label
    std::vector<std::vector<cv::Vec2f> > label_saved(numOfSP1);
    //label_saved.reserve(10000);
    std::vector<std::vector<cv::Mat_<float> > > dcost_saved(numOfSP1);
    ...
    cv::Mat_<cv::Vec<float, NUM_TOP_K> > message(height1 * width1, 4);
    message.setTo(0);
    ...
    cv::Mat_<cv::Vec4f> smoothWt(height1, width1);
...
// # pragma omp parallel for
    ...
    std::vector<cv::Vec2f> vec_label(BUFSIZE), vec_label_nei(BUFSIZE);
    std::vector<float> vec_mes_l(BUFSIZE),
                  vec_mes_r(BUFSIZE),
                  vec_mes_u(BUFSIZE),
                  vec_mes_d(BUFSIZE),
                  vec_belief(BUFSIZE),
    ...
    for (int iter = 0; iter < iterNum; iter++) {
        ...
        for (int sp = spBegin; sp != spEnd; sp += spStep) {
            ...
            cv::Vec4i curRange = subRange1[curSPP];
            int y = curRange[0];
            int x = curRange[1];
            ...
            for (sIt = sAdj.begin(); sIt != sAdj.end(); ++sIt) {
                ...
                cv::Vec2f test_label;
                //std::vector<Vec2f> & cur_label_k = label_k[repPixels1[*sIt]];
                for (int k = 0; k < NUM_TOP_K; k++) {
            ...
            for (int k = 0; k < NUM_TOP_K; k++) {
                cv::Vec2f test_label = label_k[repPixels1[sp]][k];
                float mag = std::min(disp_range_u, disp_range_v) / 8;
                for (; mag >= (1.0f / upScale); mag /= 2.0f) {
                    cv::Vec2f test_label_random;
                    float tmpVerLabel = test_label[0] + ((float(rand()) / RAND_MAX) - 0.5f) * 2.0f * mag;
                    float tmpHorLabel = test_label[1] + ((float(rand()) / RAND_MAX) - 0.5f) * 2.0f * mag;

                    test_label_random[0] = floor(tmpVerLabel * upScale + 0.5f) / upScale;
                    test_label_random[1] = floor(tmpHorLabel * upScale + 0.5f) / upScale;
            ...
            const int vec_size = (int)vec_label_nei.size();
...
#pragma omp parallel for num_threads(NTHREADS)
            ...
            cv::Vec4i curRange_s = spRange1[curSPP];
            ...
            for (int bi = spx_s; bi != spx_e; bi = bi + spx_step)
                for (int bj = spy_s; bj != spy_e; bj = bj + spy_step) {
                    ...
                    cv::Vec4f wt_s = smoothWt[bi][bj];

                    for (int k = 0; k < NUM_TOP_K; ++k) {
                        cv::Vec2f test_label = label_k[p1][k];
                    ...
                    for (int test_id = 0; test_id < vec_label_nei.size(); ++test_id) {
                        cv::Vec2f test_label = vec_label_nei[test_id];
                        //if(isNewLabel_PMF_PMBP(vec_label,test_label)==-1)
                        {
                            vec_label.push_back(test_label);
                            kx = test_id * w;
                            const cv::Mat_<float>& local = DataCost_nei(cv::Rect(kx, 0, w, h));
...
void BuildCensus_bitset(const cv::Mat_<float> &imgGray,
                        int winSize,
                        std::vector<std::vector<std::bitset<CENSUS_SIZE_OF> > >& CensusStr,
...
void spm_bp::initiateData()
{
    ...
    for (int iy = 0; iy <= CENSUS_SIZE_OF; ++iy)
        expCensusDiffTable[iy] = 1.0f - exp(-(double)iy / 30);

    for (int iy = 0; iy < 256; ++iy)
        expColorDiffTable[iy] = 1.0f - exp(-(double)iy / 60);
...
void spm_bp::init_label_super(cv::Mat_<cv::Vec2f>& label_super_k, cv::Mat_<float>& dCost_super_k) //, std::vector<std::vector<Vec2f> > &label_saved, std::vector<std::vector<Mat_<float> > > &dcost_saved)
{
    printf("==================================================\n");
    printf("Initiating particles...Done!\n");
    std::vector<cv::Vec2f> label_vec;
    ...
    for (int sp = 0; sp < numOfSP1; ++sp) {
        ...
        while (k < NUM_TOP_K) {
            float du = (float(rand()) / RAND_MAX - 0.5f) * 2 * (float)disp_range_u;
            float dv = (float(rand()) / RAND_MAX - 0.5f) * 2 * (float)disp_range_v;
            du = floor(du * upScale + 0.5f) / upScale;
            dv = floor(dv * upScale + 0.5f) / upScale;

            if (du >= -disp_range_u && du <= disp_range_u && dv >= -disp_range_v && dv <= disp_range_v) {
                int index_tp = 1;
                for (int k1 = 0; k1 < k; ++k1) {
                    if (checkEqual_PMF_PMBP(label_super_k[repPixels1[sp]][k1], cv::Vec2f(du, dv)))
                        index_tp = 0;
                }

                if (index_tp == 1) {
                    for (int ii = 0; ii < superpixelsList1[sp].size(); ++ii)
                        label_super_k[superpixelsList1[sp][ii]][k] = cv::Vec2f(du, dv);

                    label_vec.push_back(cv::Vec2f(du, dv));
                    ++k;
                }
            }
        }
        ...
        CFFilter cff;
#endif
        int vec_size = (int)label_vec.size();
...
#pragma omp parallel for num_threads(NTHREADS)
        ...
        for (int ii = 0; ii < superpixelsList1[sp].size(); ++ii) {
            ...
            for (int kk = 0; kk < NUM_TOP_K; kk++) {
                kx = kk * w;
                const cv::Mat_<float>& local = localDataCost(cv::Rect(kx, 0, w, h));
...
void spm_bp::getLocalDataCost(int sp, std::vector<cv::Vec2f>& flowList, cv::Mat_<float>& localDataCost) //, std::vector<std::vector<Vec2f> > &label_saved, std::vector<std::vector<Mat_<float> > > &dcost_saved)
{
    int dSize = (int)flowList.size();

    //USE_COLOR_FEATURES
    cv::Mat_<cv::Vec3f> subLt = subImage1[sp];
#if USE_CENSUS
    std::vector<std::vector<std::bitset<CENSUS_SIZE_OF> > > subLt_css = subCensusBS1[sp];
...
//cv::Mat_<Vec_css_bit> subLt_css_bit = subCensus_bit1[sp];
    ...
    cv::Mat_<float> rawCost(h, w);
    cv::Mat_<cv::Vec3f> subRt(h, w);
    cv::Mat_<cv::Vec2f> subRt_g(h, w);
#if USE_CENSUS
    std::vector<std::vector<std::bitset<CENSUS_SIZE_OF> > > subRt_css(h, std::vector<std::bitset<CENSUS_SIZE_OF> >(w));
#endif
...
//int check_id = isNewLabel_PMF_PMBP(label_saved[sp],flowList[kd]);
        ...
        cv::Vec3f* subLtPtr = (cv::Vec3f*)(subLt.ptr(0));
        cv::Vec3f* subRtPtr = (cv::Vec3f*)(subRt.ptr(0));
        ...
        for (iy = 0; iy < h; ++iy) {
            for (ix = 0; ix < w; ++ix) {
                ...
                std::bitset<CENSUS_SIZE_OF> tmpBS = subRt_css[iy][ix] ^ subLt_css[iy][ix];
#if USE_POINTER_WISE
...
#else
                rawCost[iy][ix] = 255 * (dist_css + dist_ce); //beta*std::min(dist_c/3,tau_c);//beta*std::min(dist_c/3,tau_c);//*255 + beta*std::min(dist_c/3,tau_c);
#endif
...
void spm_bp::getLocalDataCostPerlabel(int sp, const cv::Vec2f& fl, cv::Mat_<float>& localDataCost)
{
    //USE_COLOR_FEATURES
    cv::Mat_<cv::Vec3f> subLt = subImage1[sp];
#if USE_CENSUS
...
#pragma omp parallel for
    ...
    for (iy = 0; iy < h; ++iy) {
        for (ix = 0; ix < w; ++ix) {
            ...
            *rawCostPtr++ = 255 * (dist_css + dist_ce);
#else
            localDataCost[iy][ix] = 255 * (dist_css + dist_ce); //beta*std::min(dist_c/3,tau_c);//beta*std::min(dist_c/3,tau_c);//*255 + beta*std::min(dist_c/3,tau_c);
#endif
...
int spm_bp::createAndOrganizeSuperpixels()
{
    ...
    cv::Mat_<int> label1, label2;
    std::vector<cv::Vec4i> sub1, sub2;
    std::vector<cv::Vec4i> sp1, sp2;
...
void spm_bp::GetSuperpixelsListFromSegment(const cv::Mat_<int>& segLabels, int numOfLabels, std::vector<std::vector<int> >& spPixelsList)
{
    int iy, ix;
...
void spm_bp::RandomAssignRepresentativePixel(const std::vector<std::vector<int> >& spPixelsList, int numOfLabels, std::vector<int>& rePixel)
{
...
void spm_bp::AssociateLeftImageItselfToEstablishNonlocalPropagation(int sampleNum, int topK)
{
    ...
    const float sigmaColor = 25.0f * 25.0f;
    const float MAX_SPATIAL_DISTANCE = MAX_FL * MAX_FL * 6.25f;
    int iLt, iRt;
    for (iLt = 0; iLt < numOfSP1; ++iLt) {
        std::vector<CANDIDATE_TYPE> vecCandi;
        vecCandi.clear();
        int ltSpSize = (int)superpixelsList1[iLt].size();
        cv::Vec2i pL, pR;
        for (iRt = 0; iRt < numOfSP1; ++iRt) {
            int rtSpSize = (int)superpixelsList1[iRt].size();
            float sumWeight = 0.0;
            int iy;
            //cout<<iLt<<" "<<iRt<<endl;
...
void spm_bp::Show_WTA_Flow(int iter, cv::Mat_<cv::Vec2f>& label_k, cv::Mat_<float>& dCost_k, cv::Mat_<cv::Vec<float, NUM_TOP_K> >& message, cv::Mat_<cv::Vec2f>& flowResult)
{
    float cost_perpixel[NUM_TOP_K];
    cv::Vec2f tmp;
    //cv::Mat_<cv::Vec2f> flow_t(height1,width1);
