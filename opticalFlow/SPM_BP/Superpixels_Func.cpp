...
void GetSubImageRangeFromSegments(const cv::Mat_<int>& segLabels, int numOfLabels, int kerLen, cv::Mat_<cv::Vec4i>& subRange, cv::Mat_<cv::Vec4i>& spRange)
{
    ...
    for (iy = 0; iy < numOfLabels; ++iy) {
        subimageExtreme[iy][0][0] = std::max<int>(labelExtreme[iy][0][0] - kerLen, 0);
        subimageExtreme[iy][0][2] = std::min<int>(labelExtreme[iy][0][2] + kerLen, width - 1);
        subimageExtreme[iy][0][1] = std::max<int>(labelExtreme[iy][0][1] - kerLen, 0);
        subimageExtreme[iy][0][3] = std::min<int>(labelExtreme[iy][0][3] + kerLen, height - 1);
    }
...
void GetSubImageRangeFromSegments(const cv::Mat_<int>& segLabels, int numOfLabels, int kerLen, std::vector<cv::Vec4i>& subRange, std::vector<cv::Vec4i>& spRange)
{
    int width = segLabels.cols;
    int height = segLabels.rows;
...
void GetPCentricSubImageRange(const cv::Mat_<cv::Vec3b>& imgIn, int spLen, int kerLen, cv::Mat_<cv::Vec4i>& subRange, cv::Mat_<cv::Vec4i>& spRange)
{
    ...
    for (iy = 0; iy < height; ++iy) {
        for (ix = 0; ix < width; ++ix) {
            // 0 -- 3
            // left, up, right, down
            subRange[iy][ix][0] = std::max<int>(ix - spLen, 0);
            subRange[iy][ix][2] = std::min<int>(ix + spLen, width - 1);
            subRange[iy][ix][1] = std::max<int>(iy - spLen, 0);
            subRange[iy][ix][3] = std::min<int>(iy + spLen, height - 1);
        }
    }
    ...
    for (iy = 0; iy < height; ++iy) {
        for (ix = 0; ix < width; ++ix) {
            // 0 -- 3
            // left, up, right, down
            spRange[iy][ix][0] = std::max<int>(subRange[iy][ix][0] - kerLen, 0);
            spRange[iy][ix][2] = std::min<int>(subRange[iy][ix][2] + kerLen, width - 1);
            spRange[iy][ix][1] = std::max<int>(subRange[iy][ix][1] - kerLen, 0);
            spRange[iy][ix][3] = std::min<int>(subRange[iy][ix][3] + kerLen, height - 1);
        }
    }
}
...
void GetSegmentsRepresentativePixelsRandomAssign(const cv::Mat_<int>& segLabels, int numOfLabels, cv::Mat_<cv::Vec2i>& rePixel)
{
    ...
    rePixel.create(numOfLabels, 1);
    std::vector<std::vector<cv::Vec2i> > spPixels;
